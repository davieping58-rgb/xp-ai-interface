...
function Home({go}){
  return <ScrollView contentContainerStyle={s.home}>
    ...
    <View style={s.quickRow}>
      ...
      <Quick icon="trending-up" color="#43b8ff" title="LAST 5" sub="Recent sales" onPress={()=>go('last5')}/>
      <Quick icon="people" color={P} title="COMMUNITY" sub="Channels & chat" onPress={()=>go('community')}/>
    </View>
...
function Last5(){const[name,setName]=useState('');const[sales,setSales]=useState([]);const show=()=>{if(!name.trim())return Alert.alert('Card name','Enter a card first.');setSales([{id:1,label:'Most recent sale',price:'Price feed pending'},{id:2,label:'Sale 2',price:'Price feed pending'},{id:3,label:'Sale 3',price:'Price feed pending'},{id:4,label:'Sale 4',price:'Price feed pending'},{id:5,label:'Sale 5',price:'Price feed pending'}])};return <ScrollView contentContainerStyle={s.page}><Header title="LAST 5 SALES"/><View style={s.notice}><Ionicons name="shield-checkmark" size={28} color={GREEN}/><Text style={[s.body,{flex:1}]}>No more eBay jump. Search stays inside TattieTCG. We will show the five real sold prices here when the direct price feed is connected.</Text></View><TextInput value={name} onChangeText={setName} placeholder="Card name…" placeholderTextColor={M} style={s.input}/><Btn onPress={show}>SHOW LAST 5</Btn>{sales.map(x=><View key={x.id} style={s.saleRow}><View><Text style={s.bold}>{x.label}</Text><Text style={s.muted}>{name}</Text></View><Text style={s.salePrice}>{x.price}</Text></View>)}</ScrollView>}

function Community(){
  const channels=['general','pulls','pokemon','yu-gi-oh','trades'];
  const[channel,setChannel]=useState('general');
  const[text,setText]=useState('');
  const[messages,setMessages]=useState({
    general:[{id:1,user:'TattieTCG',text:'Welcome to the Tattie crew 🥔🔥'},{id:2,user:'PokeMaster1',text:'Who is ripping packs tonight?'}],
    pulls:[{id:3,user:'CardKing',text:'Post your best pulls in here 👀'}],
    pokemon:[{id:4,user:'PokeMaster1',text:'Pokémon chat — sets, decks and collections.'}],
    'yu-gi-oh':[{id:5,user:'YugiFan99',text:'Yu-Gi-Oh! crew checking in 🐉'}],
    trades:[{id:6,user:'TattieTCG',text:'Use this channel for trade chat. Never post private payment details.'}]
  });
  const send=()=>{const v=text.trim();if(!v)return;setMessages(m=>({...m,[channel]:[...(m[channel]||[]),{id:Date.now(),user:'You',text:v}]}));setText('')};
  return <View style={s.communityPage}><Header title="COMMUNITY"/><View style={s.communityBody}><ScrollView style={s.channelRail} contentContainerStyle={s.channelRailContent}>{channels.map(c=><Pressable key={c} onPress={()=>setChannel(c)} style={[s.channelItem,channel===c&&s.channelActive]}><Text style={[s.channelText,channel===c&&{color:W}]}># {c}</Text></Pressable>)}</ScrollView><View style={s.chatPane}><View style={s.channelHead}><Text style={s.h2}># {channel}</Text><Text style={s.muted}>TattieTCG Community</Text></View><ScrollView style={{flex:1}} contentContainerStyle={{paddingVertical:8,gap:10}}>{(messages[channel]||[]).map(m=><View key={m.id} style={s.discordMsg}><View style={s.avatar}><Text style={s.avatarText}>{m.user.slice(0,1).toUpperCase()}</Text></View><View style={{flex:1}}><Text style={s.discordUser}>{m.user}</Text><Text style={s.body}>{m.text}</Text></View></View>)}</ScrollView><View style={s.inputRow}><TextInput style={[s.input,{flex:1}]} value={text} onChangeText={setText} placeholder={`Message #${channel}`} placeholderTextColor={M} returnKeyType="send" onSubmitEditing={send}/><Pressable onPress={send} style={s.send}><Ionicons name="send" size={20} color={W}/></Pressable></View></View></View></View>
}

function Menu({go}){return <ScrollView contentContainerStyle={s.page}><Header title="MENU"/>{['My Collection','Wishlist','Price Alerts','Trade List'].map(x=><Pressable key={x} style={s.menuLine} onPress={()=>Alert.alert(x,'This section is ready for your saved collection data.')}><Text style={s.bold}>{x}</Text><Ionicons name="chevron-forward" size={20} color={P}/></Pressable>)}<Btn onPress={()=>go('tattie')}>TALK TO WEE TATTIE AI</Btn><Btn outline onPress={()=>go('qr')}>QR DOWNLOAD</Btn><Btn outline onPress={()=>Linking.openURL(APK)}>CHECK FOR APP UPDATE</Btn><Text style={[s.muted,{textAlign:'center'}]}>Updates use the clean Tattie download route — no GitHub account shown.</Text><Text style={[s.muted,{textAlign:'center'}]}>DOMHNALL • Making Technology Available For All</Text></ScrollView>}
...
const s=StyleSheet.create({
 ...
 saleRow:{borderWidth:1,borderColor:LINE,borderRadius:16,backgroundColor:CARD,padding:13,flexDirection:'row',justifyContent:'space-between',gap:10,alignItems:'center'},salePrice:{color:BLUE,fontWeight:'800',fontSize:12,textAlign:'right',maxWidth:'45%'},
 communityPage:{flex:1,backgroundColor:BG,paddingHorizontal:10,paddingTop:6},communityBody:{flex:1,flexDirection:'row',gap:8,paddingBottom:8},channelRail:{width:105,backgroundColor:'#0b0710',borderRadius:14,borderWidth:1,borderColor:LINE},channelRailContent:{padding:8,gap:6},channelItem:{paddingVertical:10,paddingHorizontal:7,borderRadius:9},channelActive:{backgroundColor:'#3b2350'},channelText:{color:M,fontWeight:'800',fontSize:11},chatPane:{flex:1,backgroundColor:'#0d0912',borderRadius:14,borderWidth:1,borderColor:LINE,padding:10},channelHead:{borderBottomWidth:1,borderBottomColor:LINE,paddingBottom:9},discordMsg:{flexDirection:'row',gap:9,alignItems:'flex-start'},avatar:{width:34,height:34,borderRadius:17,backgroundColor:P2,alignItems:'center',justifyContent:'center'},avatarText:{color:W,fontWeight:'900'},discordUser:{color:'#d7b5ff',fontWeight:'900',fontSize:13,marginBottom:2},
