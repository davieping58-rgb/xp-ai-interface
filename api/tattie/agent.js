...
export default async function handler(req, res) {
  ...
  try {
    ...
    const userContext = profile.name
      ? `The customer's preferred first name is ${String(profile.name).slice(0, 40)}. Use it naturally, not in every reply.`
      : '';
    ...
    const input = [
      ...
      ...history
        ...
        .map(x => ({ role: x.role, content: [{ type: 'input_text', text: x.content.slice(0, 1800) }] })),
